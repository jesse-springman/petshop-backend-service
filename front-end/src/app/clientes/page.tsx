...
export default function GetClients() {
    ...
    const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001/clientes";
    ...
    useEffect(() => {
        ...
        const getDataClients = async () => {
            try {
                ...
                setErro("Não foi possível localizar os clientes, por favor tente novamente mais tarde.");
                console.log(error);
            } finally {
                setLoading(false);
            }
        ...
        <main className="min-h-screen bg-gradient-to-b from-[#0B0E11] to-[#1A1D22] flex items-center justify-center p-4">
            <div className="bg-[#1A1D22] p-8 md:p-12 rounded-2xl shadow-2xl border border-amber-500/20 max-w-4xl w-full">
                ...
                {!loading && !erro && clients.length === 0 && (
                    <p className="text-center text-gray-400 text-xl">Nenhum cliente cadastrado ainda</p>
                )}

                {!loading && !erro && clients.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {clients.map((client: Client, index: number) => (
                            <div className="bg-[#0B0E11] p-6 rounded-xl border border-amber-500/20 shadow-md hover:shadow-lg transition"
                                key={client.id ?? index}>
